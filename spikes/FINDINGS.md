...
Profile allowing 4 skills launched Grok with 9. Turning a skill off therefore means ignoring
...
`explain` a `baseline = "none"` Profile, launch, and count what `grok inspect --json` reports.
...
there is no skill or plugin toggle at all. Every Profile decision would have been a warning.
...
before trusting a Profile that turns a Grok plugin off.
...
reports a symlink's own dirent type, not its target's, so `entry.isDirectory()` is `false` for
