...
```bash
# 1. Annotate a real npx-skills-managed skill (via `plugged-in adopt` once it exists, or by
...
If clobbered, `plugged-in doctor` (Phase 5) must detect this as drift and `adopt` must be
