...
| **Profile** | A named, reusable on/off decision: a Baseline plus `allow`/`deny` deltas. |
| **Baseline** | What a Profile starts from: `all`, `none`, or another Profile by name. |
| **Resolution** | The result of resolving a Profile against a live Inventory: a concrete on/off decision for every discovered Component. |
...
```
Inventory discovery  →  Profile resolution  →  Projection  →  Activation (run)
...
`settings.json`/`config.toml` by hand, so that marketplace/config-merging logic stays the
...
**2. Profile resolution** (`src/profile/resolve.ts`). Pure function:
`(Profile, Inventory, all-known-Profiles) → Resolution`. Starts from the Profile's Baseline
(recursively resolving through a chain of `baseline: <other profile>` references, erroring on
a cycle) and applies `allow`/`deny` on top. Because it resolves against the *live* Inventory
every time rather than a snapshot, a `baseline: all` Profile automatically picks up a newly
...
Projection can **refuse** instead of guessing, and separately can **warn**, using the same
underlying gap (a Component can't be faithfully turned to the state a Profile wants), but two
...
- **Warns (non-blocking, launch proceeds, the Component is left as the Client's own config
  ...
  *through this tool*, since the gap is permanent rather than actionable, so blocking every launch
  forever would just force every Profile to explicitly `allow` every pre-existing Codex MCP
...
- **A Profile** is a TOML file, nothing else. `~/.plugdin/profiles/<name>.toml` (global)
  or `<project>/.plugdin/profiles/<name>.toml` (project). Same name in both places → the
  project one wins outright; they never merge.
- **A project's default Profile** (used when `run` is invoked with no `--profile` and no one
  around to ask) is one line in `<project>/.plugdin/config.toml`: `default_profile = "x"`.
...
src/profile/                stage 2: resolve.ts (pure), store.ts (read TOML), write.ts (write TOML)
...
src/tui/                    the interactive Profile picker's I/O layer
