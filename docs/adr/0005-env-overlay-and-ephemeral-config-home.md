...
Pi needed neither. Its CLI already expresses a Loadout directly — `--no-skills` /
...
real config home never modified or relinked. Two terminals can run different Loadouts at
...
- **Warn on everything Grok cannot express through flags** — rejected: it would make Grok
  ...
  plugins, or MCP servers at all. Every Loadout targeting Grok would be advisory.
...
- **Support Cursor CLI in this batch** — dropped rather than faked. Cursor reads `mcp.json`
  ...
  whose every Loadout decision is a warning. It can be revisited if Cursor grows a
