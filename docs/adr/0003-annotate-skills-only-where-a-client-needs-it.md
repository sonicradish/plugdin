...
no such shim, since `skills.config = [{ name = "…", enabled = false }]` removes a skill from
...
carries no information about which Profile is active. Activation still happens entirely
...
- **Leave loose skills ungoverned**: rejected once Profiles were defined as context
