# Project Profiles through ephemeral config and native client flags
...
the user's real configuration directories. A crashed or killed plugdin therefore cannot
leave a machine in a wrong state, and two terminals can run different Profiles at once.
...
- **Config-home redirect**: synthesize a whole `CLAUDE_CONFIG_DIR` / `CODEX_HOME` per
  Profile. Rejected: credentials, history, and project state live there too, so the
