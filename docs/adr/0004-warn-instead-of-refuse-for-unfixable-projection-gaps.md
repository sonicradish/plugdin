...
gap nobody can close doesn't protect anything; it just forces every Profile that targets
...
- **Keep it a hard refusal indefinitely**: rejected: since no fix can ever close it, this
  ...
  is `allow`-ing the Component explicitly in every affected Profile, which just re-encodes
...
- **Drop the gap silently (no message at all)**: rejected outright: contradicts the tool's
  entire premise. The person launching must always be able to see where a Profile's stated
...
Profile with only warnings exits `0`. If a second permanently unfixable gap ever surfaces,
