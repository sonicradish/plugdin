# pluggedin selects Components; it does not install them
...
`npx skills` does the same for skills. pluggedin reads the resulting Inventory and decides
...
The Inventory is discovered, not owned, so pluggedin must tolerate Components appearing and
