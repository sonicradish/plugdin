# plugged-in selects Components; it does not install them
...
`npx skills` does the same for skills. plugged-in reads the resulting Inventory and decides
...
The Inventory is discovered, not owned, so plugged-in must tolerate Components appearing and
