...
- **Fail closed, but only when asked.** A new Profile starts at baseline `none`. Running the
  wrapper without naming a Profile uses the project default, else `all`. Not using plugdin
...
| S2 | Does `--strict-mcp-config` also suppress plugin-provided MCP servers? | Profiles that enable a plugin could silently lose its server |
...
MCP servers, with user-pinnable aliases), and print what a given Profile *would* produce:
...
TOML at `~/.plugdin/profiles/<name>.toml` and `<repo>/.plugdin/profiles/<name>.toml`,
committed. Project overrides global **by name**, never merges, because merged allow/deny sets across
scopes cannot be debugged. A project may declare a default Profile; it may never force one.

A Profile is a set of allows and denies over a Baseline of `all`, `none`, or another Profile.
Resolution is a function of the live Inventory, so a `baseline: all` Profile picks up newly
...
`plugdin run <client> [--profile X] [native args…]`. Argument passthrough is a hard
requirement from day one: reserve `--profile` and treat everything else as opaque. The moment
the wrapper cannot accept `-p` or `--model`, people stop using it. TUI picker when no Profile
